...
setup_linux_kernel() {
	...
	*)
	...
	if [ ! -d "$BAO_DEMOS_LINUX_SRC" ]; then
		...
		git clone $BAO_DEMOS_LINUX_REPO "$BAO_DEMOS_LINUX_SRC" --depth 1 --branch $BAO_DEMOS_LINUX_VERSION
	fi
	...
	if [ "$patches" == "1" ]; then
		...
		git -C "$BAO_DEMOS_LINUX_SRC" apply \
			"$BAO_DEMOS_LINUX/patches/$BAO_DEMOS_LINUX_VERSION/"*.patch
	fi
...
buildroot-$ARCH-$BAO_DEMOS_LINUX_VERSION"
	...
	if [ ! -d "$BAO_DEMOS_BUILDROOT" ]; then
		...
		git clone $repo "$BAO_DEMOS_BUILDROOT" --depth 1 --branch $branch
	fi
	#	cd "$BAO_DEMOS_BUILDROOT" || (echo "Missing $BAO_DEMOS_BUILDROOT" && return 1)

	if [ "$bt" == "clean" ]; then
		echo "cleaning previous build..."
		make -C "$BAO_DEMOS_BUILDROOT" distclean
	fi
	...
	if [ "$bt" == "clean" ] || [ "$bt" == "deep" ]; then
		...
		echo ">> Building config: $BAO_DEMOS_BUILDROOT_DEFCFG"
		make -C "$BAO_DEMOS_BUILDROOT" \
			defconfig \
			BR2_DEFCONFIG="$BAO_DEMOS_BUILDROOT_DEFCFG"
		print_info "$BR2_DEFCONFIG"
		read -rp "Press to build buildroot: " user_input
		# Building
		ignore_error=true
		make_cmd="make -C $BAO_DEMOS_BUILDROOT linux-reconfigure all"
		run_make_cmd "$make_cmd"

		ignore_error=false
		mv -v "$BAO_DEMOS_BUILDROOT/output/images/Image" \
...
wrap_kernel_dtb() {
	...
	make_cmd="make -C $BAO_DEMOS_LINUX/lloader\
	 ARCH=$ARCH	 IMAGE=$BAO_DEMOS_BUILDROOT/output/images/Image-$PLATFORM	 DTB=$BAO_DEMOS_WRKDIR_IMGS/$BAO_DEMO_LINUX_VM.dtb\
